...
public class main {
	public static void main(String[] args) {
		...
		while (gameOver == false)// Game loop
		{
			...
			// Using the x and y val it attempts to set that space to an X or O based on
			// whose turn it is.
			if (board.setSpace(xVal, yVal, player1Turn ? TicTacToeBoard.X_SPACE : TicTacToeBoard.O_SPACE)) {
				...
				if (board.checkVictory(player1Turn)) {
					board.drawBoard();
				...
				// If a space wasn't empty or was out of the board range then prompt with an
				// error
